...
BEGIN
    ...
    IF poor_brix IS NULL OR excellent_brix IS NULL OR excellent_brix = poor_brix THEN
        RETURN 1.5;
    END IF;

    RETURN 1.0 + LEAST(1.0, GREATEST(0.0,
        (brix_value_arg - poor_brix) / (excellent_brix - poor_brix)
    ));
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION public.get_brand_leaderboard(
    ...
    state_filter text,
    country_filter text -- Assumes a "country" column exists in the places table
) RETURNS TABLE(
    brand_id uuid,
    brand_name text,
...
GROUP BY rc.crop_id, rc.crop_name, rc.submission_count, rc.average_brix, rc.average_normalized_score, rc.grade, rc.grade_rank_sort
ORDER BY rank;
$function$;
