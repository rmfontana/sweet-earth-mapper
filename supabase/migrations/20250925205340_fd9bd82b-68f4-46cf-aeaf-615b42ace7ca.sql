-- Create a safer user leaderboard function that doesn't need to access the users table
-- This avoids RLS issues by using contributor_name from submissions directly
CREATE OR REPLACE FUNCTION public.get_user_leaderboard_safe(
  ...
  SELECT 
    ...
    ROW_NUMBER() OVER (ORDER BY us.avg_normalized_score DESC, us.total_submissions DESC) as rank
  FROM user_stats us
  ORDER BY rank;
$$;

-- Grant execute permissions to anon and authenticated roles
GRANT EXECUTE ON FUNCTION public.get_user_leaderboard_safe(text, text, text, text) TO anon;
GRANT EXECUTE ON FUNCTION public.get_user_leaderboard_safe(text, text, text, text) TO authenticated;