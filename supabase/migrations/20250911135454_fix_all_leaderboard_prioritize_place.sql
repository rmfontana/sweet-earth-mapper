--
-- Drop existing leaderboard functions to replace them with the new versions
--
...
WITH normalized_brand_crop_scores AS (
    ...
    LEFT JOIN locations l ON p.location_id = l.id
    JOIN crops c ON s.crop_id = c.id
    WHERE (place_id_filter IS NULL OR s.place_id = place_id_filter)
      AND (location_name_filter IS NULL OR l.name = location_name_filter)
