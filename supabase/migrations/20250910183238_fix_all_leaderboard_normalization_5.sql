CREATE OR REPLACE FUNCTION public.get_crop_leaderboard(
 ...
 RETURNS TABLE(crop_id uuid, crop_name text, submission_count bigint, average_brix numeric, average_normalized_score numeric, rank bigint)
 LANGUAGE sql
 STABLE
AS $function$
...
SELECT
    ...
    rc.submission_count,
    rc.average_brix,
    rc.avg_normalized_score,
    RANK() OVER (ORDER BY rc.avg_normalized_score DESC, rc.submission_count DESC) as rank
...
CREATE OR REPLACE FUNCTION public.get_brand_leaderboard(
 ...
 RETURNS TABLE(brand_id uuid, brand_name text, submission_count bigint, average_brix numeric, average_normalized_score numeric, rank bigint)
 LANGUAGE sql
 STABLE
AS $function$
...
SELECT
    ...
    rb.submission_count,
    rb.average_brix,
    rb.avg_normalized_score,
    RANK() OVER (ORDER BY rb.avg_normalized_score DESC, rb.submission_count DESC) as rank
...
CREATE OR REPLACE FUNCTION public.get_location_leaderboard(
 ...
 RETURNS TABLE(location_id uuid, location_name text, submission_count bigint, average_brix numeric, average_normalized_score numeric, rank bigint)
 LANGUAGE sql
 STABLE
AS $function$
...
SELECT
    ...
    rl.submission_count,
    rl.average_brix,
    rl.avg_normalized_score,
    RANK() OVER (ORDER BY rl.avg_normalized_score DESC, rl.submission_count DESC) as rank
