-- Drop and recreate crop leaderboard function with proper grade calculation
...
WITH ranked_crops AS (
        ...
        AVG(
            CASE
                WHEN c.excellent_brix IS NOT NULL AND c.poor_brix IS NOT NULL
                    ...
                    -- Normalize between poor (0) and excellent (1)
                    LEAST(1.0, GREATEST(0.0,
                         (s.brix_value - c.poor_brix) / (c.excellent_brix - c.poor_brix)
                    ))
                ELSE
                    0.5 -- Default normalized score when thresholds are missing or invalid
            END
        ) AS average_normalized_score,
        -- Calculate grade based on average brix relative to crop thresholds
        CASE
    ...
    WHERE s.crop_id IS NOT NULL
      ...
      AND (location_name_filter IS NULL OR l.name = location_name_filter)
...
-- Drop and recreate brand leaderboard function with proper grade calculation
...
ranked_brands AS (
    SELECT
        ...
        bsd.average_brix,
        -- Final average is the average of the crop-specific normalized scores
        AVG(n.average_normalized_score_for_crop) AS average_normalized_score,
        -- Calculate grade based on this final average normalized score
        CASE
...
-- Drop and recreate location leaderboard function with proper grade calculation
