--
-- Corrected get_brand_leaderboard function
--
...
BEGIN
  ...
  brand_scores AS (
    SELECT
      ...
      -- Renaming the column here to avoid the conflict
  ...
  ranked AS (
    SELECT
      wg.*,
      -- Reference the temporary name in the ORDER BY clause
      RANK() OVER (ORDER BY wg.avg_score_temp DESC, wg.grade DESC) AS rank
    FROM with_grades wg
  )
  -- Select all columns from the final CTE, and alias the temporary name back
  SELECT
    ranked.brand_id,
    ranked.brand_name,
...
--
-- Corrected get_crop_leaderboard function
--
...
BEGIN
  ...
  crop_scores AS (
    SELECT
      c.id AS crop_id,
      c.name AS crop_name,
      -- Renaming the column to avoid the conflict
      AVG(fs.normalized_score) AS avg_score_temp,
      COUNT(*) AS submission_count
    FROM filtered_submissions fs
...
--
-- Corrected get_location_leaderboard function
--
